function [ Proj ] = estimateCameraProjRANSAC( points3D, points2D,M )
...
for i = 1 : 1000
    ...
    Proj_six = six_points( points3D(random, :), points2D(random, :),M);
        ...
        if sum(inlier) > inlier_num
            ...
            Proj = six_points( points3D(inlier, :), points2D(inlier, :),M);
